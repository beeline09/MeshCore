// Darktec overlay of examples/companion_radio/ui-new/UITask.cpp
// (message history + UTF-8-safe preview). Original examples/ file is unchanged.
#include "UITask.h"
...
// Copy UTF-8 without splitting a multi-byte character (avoids garbage glyphs on OLED).
static void utf8_strncpy(char* dest, const char* src, size_t buf_sz) {
  if (!dest || buf_sz == 0) return;
  size_t o = 0;
  size_t i = 0;
  while (src[i] && o + 1 < buf_sz) {
    ...
    if (o + clen >= buf_sz) break;
    memcpy(dest + o, src + i, clen);
    o += clen;
    i += clen;
  }
  dest[o] = 0;
...
class MsgPreviewScreen : public UIScreen {
  ...
  void pushLog(const MsgEntry& entry) {
...
public:
  ...
  void addPreview(uint8_t path_len, const char* from_name, const char* msg) {
    ...
    utf8_strncpy(p->msg, msg, sizeof(p->msg));
    pushLog(*p);
    _in_history = false;
  }
  ...
  void enterHistoryMode() {
    _in_history = true;
    _hist_cursor = _log_count > 0 ? _log_count - 1 : 0;
  }

  int render(DisplayDriver& display) override {
    const MsgEntry* p = nullptr;
    char tmp[24];
    if (_in_history) {
      if (_log_count <= 0) {
        _in_history = false;
      ...
      if (_hist_cursor < 0) _hist_cursor = 0;
      if (_hist_cursor >= _log_count) _hist_cursor = _log_count - 1;
    ...
    display.setColor(UIColor::corp_blue);
    if (_in_history) {
      snprintf(tmp, sizeof(tmp), "%d/%d", _hist_cursor + 1, _log_count);
    } else {
  ...
  bool handleInput(char c) override {
    ...
    if (c == KEY_ENTER) {
      num_unread = 0;  // clear unread queue; history keeps a copy
      _task->gotoHomeScreen();
      return true;
    }
    return false;
...
void UITask::gotoMsgPreview() {
  if (!msg_preview) return;
  if (getUnreadMsgCount() > 0) setCurrScreen(msg_preview);
}
