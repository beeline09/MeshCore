#!/usr/bin/env bash
# Build all Darktec official roles × battery chemistry into out/.
# Filenames: DarktecOff_{role}_{chem}_{cells}s.{uf2,zip}
# No battery-protect matrix — chemistry is UI % only.
#
# Optional filters (CI shards / local subset):
#   DARKTEC_ROLE_SLUG=companion_radio_ble
#   DARKTEC_PIO_ENV=Darktec_companion_radio_ble
# When unset, builds the full 8×4 = 32 matrix.
#
...
build_one() {
  ...
  local chem_slug="$4"
  local cells="$5"
  local out_base="DarktecOff_${role_slug}_${chem_slug}_${cells}s"
  ...
  export PLATFORMIO_BUILD_FLAGS="-DBATTERY_CHEMISTRY=${chem_macro} -DBATTERY_CELLS=${cells} -DCFG_DEBUG=0 -DFIRMWARE_VERSION='\"${FIRMWARE_VERSION_STRING}\"' -include ${QUIET_H}"
...
echo "=== Darktec official matrix done ==="
ls -la out/DarktecOff_*_*s.uf2

flasher_count="$(find out -maxdepth 1 -type f \( \
  -name 'DarktecOff_*_liion_1s.uf2' -o \
  -name 'DarktecOff_*_lifepo4_1s.uf2' -o \
  -name 'DarktecOff_*_lto_1s.uf2' -o \
  -name 'DarktecOff_*_lto_2s.uf2' \
\) | wc -l | tr -d ' ')"
