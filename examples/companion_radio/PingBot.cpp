...
/* ---- MyMesh members that belong to this role; kept here to stay out of MyMesh.cpp ---- */

// autoadd_config bits, mirrored from MyMesh.cpp where they are #define'd locally rather than
// exported in a header. Keep in sync if that list ever changes.
...
bool MyMesh::sendPingBotReply(uint8_t channel_idx, const char* text) {
  ...
  send_unscoped = _ping_bot_scope.isNull();  // no region: don't fall back to the default scope
